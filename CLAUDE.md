...
- **Session picking**: both `sc resume` and `sc close` source from `session.ListForRepo` via `internal/sessionpick` (`Choose` for close, `ChooseAutoSingle` for resume, which short-circuits a lone candidate past the picker) — tab completion (`completeWorktreeTargets`) and the picker use the same list, sorted active-first by `session.SortStates`, with completion labels reusing the picker rows' Detail strings so both read identically. The resume picker (only — close stays local-only) appends cached remote rows from `remote.ReadAllCaches` after the local rows (`Item.State` nil, `Item.Target` = `host:<id>`); selecting one routes over the remote attach template with no dialog, and remote rows never count toward the lone-candidate shortcut. Non-TTY callers get an error listing IDs instead of a hung prompt. Orphaned git worktrees without a state file are not valid `sc close` targets; remove them with `git worktree remove`.
...
- **External tool deps**: `git` is always required and resolved from `PATH`. `madder` and `direnv` are runtime deps **only** when the binary was built via `lib.mkSpinclass` with the matching input — those paths are burned in at link time (see `spinclass-build-pins(7)` and FDR 0003); the default `nix build` produces a binary with both pins empty, in which case the madder integration is dormant and direnv falls back to `PATH`. `clown` is an optional runtime dep for job-wakeup emits (`internal/clown`): chat wake emits AND async merge/check job-lifecycle emits both fire when `$CLOWN_BIN` is set (the running-under-clown signal) and are dormant otherwise; `SPINCLASS_CHAT_WAKE=clown` gates only the receive side (chat-watch stand-down) — see FDR 0009's migration section (incl. the mixed-window incident that split the axes) and the async section below. Interactive prompts use the in-process `huh` library (no `gum` dependency).
