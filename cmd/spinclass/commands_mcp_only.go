...
func registerMCPOnlyCommands(app *command.App) {
	...
	app.AddCommand(&command.Command{
		...
		Description: command.Description{
			Short: "Spawn a detached, harness-booted worker session (FDR 0006). `repo` is OPTIONAL (#262): omit it — or name the current repo — to spawn in THIS repo (a fresh worktree off the default branch), else a sibling dirname/path. ASYNC (#266): this returns the worker's session_key (= its chat address for chat-send), worktree_path, and a ringmaster job id IMMEDIATELY — it does NOT block. The worker's SessionStart hello is delivered as a job-wakeup ([clown-job]); the hello-timeout defaults to 5m (cold nix boots are costless async), overridable via hello-timeout. End your turn and let the wake arrive — do not poll. On hello the worker is up and will message you via chat. On timeout a dead worker is auto-reaped (nothing of value exists pre-hello), while a still-booting one is left dangling and named in the wake to reap with close-child-session. The brief is the worker's ONLY context: include everything it needs plus an explicit 'message me back at <your session key> via chat when done' instruction. A spawned worker MAY spawn its own workers — the former one-level restriction is gone — but note that `close-child-session` authorizes on immediate parentage only, so a worker you spawn indirectly is not yours to reap.",
		},
		Annotations: &protocol.ToolAnnotations{
			ReadOnlyHint:    protocol.BoolPtr(false),
			DestructiveHint: protocol.BoolPtr(false),
...
// targetsParamDesc is shared by merge-this-session and its -async twin (FDR
