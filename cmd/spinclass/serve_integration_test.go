...
func TestServeCheckThisSessionHiddenByDefault(t *testing.T) {
	...
	// The *-async tools and session-job-cancel are clown-gated (#23), and this
	// serve runs with CLOWN_BIN="" (no clown); session-job-status was retired
	// outright. None of them may be registered here.
	for _, gone := range []string{
		"merge-this-session-async", "check-this-session-async",
		"session-job-cancel", "session-job-status",
	} {
		if names[gone] {
			t.Errorf("%q must not be registered without clown (session-job-status not at all, post-#23)", gone)
		}
	}
}

// The async lifecycle (start → succeed / fail / cancel → aborted, hook
// subprocess kill) is covered at the unit level in internal/job (wake_test.go,
// job_test.go) with a stubbed ringmaster. The former serve-level async tests
// were dropped in #23: the *-async tools are now registered only under clown,
// so a clown-absent serve (as these tests ran) no longer registers them, and
// session-job-status — which they polled — was retired.
