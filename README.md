...
- **`[hooks]`**: `create` / `stop` / `on-attach` / `on-detach` / `pre-merge` / `repair` lifecycle hooks, plus `disable-merge`, `disable-repair`, `disable-nix-gc`, and `disable-merge-build-worktree` flags. By default the `pre-merge` hook runs in a transient detached build worktree pinned to the committed sha being merged (freeing the session worktree for concurrent work); `disable-merge-build-worktree = true` runs it in place. `repair` (e.g. `conformist --commit --amend --exit-zero-on-fix`) runs before the `pre-merge` VERIFY hook to auto-fold mechanical fixes into the merged commit (FDR 0018).
