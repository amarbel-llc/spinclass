...
4. **Revoke** at `close.RunResolved` (`sc close` / `close-child-session`), before the worktree is removed; at `clean.removeWorktree` (`sc clean`'s merged-worktree reaping had no teardown hook); and at the merge's own out-of-session teardown (`merge.teardownAndPush`, the `sc merge <target>` / `sc run` path that removes the worktree without tombstoning — found by the live round-trip, where the merged session's token would otherwise have been orphaned to the issuer's sweep). Best-effort: a failed revoke is a `severity=warn` point, the credential file is left, and the token stays recorded unrevoked for the sweeper. A successful revoke removes the file and stamps `credential.revoked_at` on the state (live file or tombstone).
5. **Sweep** orphaned tokens (crashed / never-closed sessions): at the next session creation on the repo, `auth.SweepOrphans` runs the revoke command for every tombstoned or abandoned session of that repo whose credential record has no `revoked_at`, with that session's identity env, and stamps the tombstone. Best-effort per session; failures are reported and left for the next sweep. papi (papi#73) enforces a TTL sweep as the backstop. The sweep revokes **by session id**, so a session key reused after a tombstone (a chosen branch name, `sc fork`) would otherwise let a stale record revoke the new session's token — the creation funnel therefore runs the sweep **before** the mint, and the new session's `session.Write` replaces the tombstone, so a stale record is revoked and retired before a token can exist under that id.
...
- **Forge-side verification of the credential shape** (circus/keen-aspen, Forgejo v15.0.7 `services/auth/method/basic.go`): with a non-empty password that is not `x-oauth-basic`, the password is looked up as the token and the acting user comes from the token's owner — the decoded username is only trace-logged, never compared (an empty password makes the username the token instead; either slot works). The Basic method fires on the git smart-HTTP / raw / LFS paths, so fetch and push are covered. Load-bearing for the per-repo promise: the PAT branch feeds `authz.GetAuthorizationReducerForAccessToken`, i.e. the v15 fine-grained resource enforcement (listed repo write, unlisted-public read-only) applies on this git-over-HTTPS path, not only on API calls. No forge configuration is needed.
...
- **Promotion evidence (2026-09-03T10:29:29Z):** a fresh session on spinclass 0.1.42+b879e35 minted a papi per-repo token at creation and landed this very commit through the FDR 0029 landing worktree with it (live round-trip, operator present).
