// Code generated by tommy with manual additions for [[start-commands]].
...
func decodeStartCommands(
	...
	for i, node := range nodes {
		...
		if v, err := document.GetFromContainer[[]string](doc, node, "exec-completions"); err == nil {
			data.StartCommands[i].ExecCompletions = v
			consumed[keyPrefix+"start-commands.exec-completions"] = true
		}
		if v, err := document.GetFromContainer[[]string](doc, node, "exec-start"); err == nil {
...
func encodeStartCommands(doc *document.Document, data *Sweatfile) error {
	for i := range data.StartCommands {
		...
		if sc.ExecCompletions != nil {
			if err := doc.SetInContainer(entry, "exec-completions", sc.ExecCompletions); err != nil {
				return err
			}
		}
		if sc.ExecStart != nil {
			if err := doc.SetInContainer(entry, "exec-start", sc.ExecStart); err != nil {
				return err
			}
		}
	}
