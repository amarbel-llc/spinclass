...
func runResolved(t *testing.T, mock *mockExecutor, repoDir, wtPath, branch, defaultBranch string, gitSync, inSession bool) ([]ndjsoncrap.Record, error) {
	...
	ts := rep.TestStream(0)
	_, err := Resolved(mock, rep, ts, repoDir, wtPath, branch, defaultBranch, gitSync, inSession, PostMergeOptions{})
	ts.Finish()
...
func TestPrepareMergePinsShaAcrossConcurrentCommit(t *testing.T) {
	...
	rep := crap.NewReporter(&buf, crap.ReporterOptions{})
	ts := rep.TestStream(0)
	pinnedSha, err := PrepareMerge(ts, repoDir, wtPath, "feature-pin", "main", false, PostMergeOptions{})
	if err != nil {
	...
	if _, err := FinishMerge(context.Background(), &mockExecutor{}, rep, ts,
		repoDir, wtPath, "feature-pin", "main", pinnedSha, false, true, nil, PostMergeOptions{}); err != nil {
...
func TestPrepareMergeHaltsOnPostRebaseConflict(t *testing.T) {
	...
	rep := crap.NewReporter(&buf, crap.ReporterOptions{})
	ts := rep.TestStream(0)
	_, err := PrepareMerge(ts, repoDir, wtPath, "feature-conflict", "main", false, PostMergeOptions{})
	if err == nil {
...
func TestPrepareMergeEmitsCoActiveSessionsPoint(t *testing.T) {
	...
	rep := crap.NewReporter(&buf, crap.ReporterOptions{})
	ts := rep.TestStream(0)
	if _, err := PrepareMerge(ts, repoDir, wtPath, "feature-co", "main", false, PostMergeOptions{}); err != nil {
		t.Fatalf("PrepareMerge: %v\n%s", err, buf.String())
	}
	ts.Finish()
...
func TestPrepareMergeNoCoActivePointWhenNone(t *testing.T) {
	...
	rep := crap.NewReporter(&buf, crap.ReporterOptions{})
	ts := rep.TestStream(0)
	if _, err := PrepareMerge(ts, repoDir, wtPath, "feature-solo", "main", false, PostMergeOptions{}); err != nil {
		t.Fatalf("PrepareMerge: %v\n%s", err, buf.String())
	}
	ts.Finish()
