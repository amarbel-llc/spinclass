...
func TestFinishMergeQueueRebasesLandingAndGatesOnLandingSha(t *testing.T) {
	...
	_, err := FinishMerge(context.Background(), &mockExecutor{}, rep, ts,
		repoDir, wtPath, "feature-race", "main", pinnedSha, false, true, nil, PostMergeOptions{})
	ts.Finish()
	if err != nil {
...
func TestFinishMergeQueueIntegrationConflict(t *testing.T) {
	...
	_, err := FinishMerge(context.Background(), &mockExecutor{}, rep, ts,
		repoDir, wtPath, "feature-conflict-race", "main", pinnedSha, false, true, nil, PostMergeOptions{})
	ts.Finish()
	if err == nil {
...
func TestFinishMergeQueueDisabledKnobFailsAtFF(t *testing.T) {
	...
	_, err := FinishMerge(context.Background(), &mockExecutor{}, rep, ts,
		repoDir, wtPath, "feature-knob", "main", pinnedSha, false, true, nil, PostMergeOptions{})
	ts.Finish()
	if err == nil {
...
func TestFinishMergeQueueRebasedLandingTeardown(t *testing.T) {
	...
	_, err := FinishMerge(context.Background(), &mockExecutor{}, rep, ts,
		repoDir, wtPath, "feature-teardown", "main", pinnedSha, false, false, nil, PostMergeOptions{})
	ts.Finish()
	if err != nil {
		t.Fatalf("FinishMerge: %v\n%s", err, buf.String())
	}
...
func TestFinishMergeQueuePostPinCommitsSurviveRebasedTeardown(t *testing.T) {
	...
	_, err := FinishMerge(context.Background(), &mockExecutor{}, rep, ts,
		repoDir, wtPath, "feature-postpin", "main", pinnedSha, false, false, nil, PostMergeOptions{})
	ts.Finish()
	if err != nil {
		t.Fatalf("FinishMerge: %v\n%s", err, buf.String())
	}
